 ...
 */
export async function parseModelManifest(
  modelPath: string,
  modelsRoot: string,
): Promise<Model> {
  return (await invokeCommand("parse_model_manifest", {
    modelPath: modelPath,
...
export async function scanModelsDirectory(
  ...
  return (await invokeCommand("scan_models_directory", {
    modelsRoot: modelsRoot,
  })) as Promise<Model[]>;
}
...
export async function saveModelLibrary(
  ...
  await invokeCommand("save_model_library", {
...
export async function loadModelLibrary(libraryPath: string): Promise<Model[]> {
  return (await invokeCommand("load_model_library", {
    libraryPath: libraryPath,
  })) as Promise<Model[]>;
}
...
export function parseModelIdentifier(identifier: string): {
  ...
  if (parts.length !== 3) {
    throw new Error(
      "Invalid model identifier format. Expected: provider:name:version",
    );
  }
  return {
    provider: parts[0],
